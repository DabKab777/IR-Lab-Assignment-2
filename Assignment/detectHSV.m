...
%Red
% if a colour centroid is found 
    if(isnan(centroids(1,1))==false)
        % fidn the depth value at the found pixel coordinate by fidn the
        % depth value of pixel coordinate in the depth image. This work
        % Because the RGB adn depth image are the same size
...
BOT_COORDS = [RED_BOT_COORDS,GREEN_BOT_COORDS,BLUE_BOT_COORDS] % returns the coordinates of the blocks in the robot's coordinate system. NOTE: final offsets are added in the dobot_code 
return 
...
mask  = imclose(mask, structuringElement); %TBD
...
coords = zeros(1,3)
%319.239 & 236.959 are the principal points of hte camera
% 386.557 is the focal length of hte camera
X = ((x_pixel - 319.239)*int16(depth_measurement))/386.556  % from tutorial 2 of sensors and control to obtain X & Y coordinatoes
...
BOT_Z= ((int16(depth_measurement)-Z_off)/1000)*-1   % need to cast depth measurement at int16 as it is a uint16 and therefore can't be less than 0
