classdef RobotBaseClassNoColour < handle
...
%% General class for multiDOF robot simulation
        function self = RobotBaseClassNoColour()
...
%% PlotAndColourRobot
        ...
        % end
        %% PlotAndColourRobot
% Given a robot index, add the glyphs (vertices and faces) 
function PlotAndColourRobot(self)
    if isempty(self.homeQ)
        self.homeQ = zeros(1,self.model.n);
    end
...
% With this modified function,  avoid any issues related to colors from the PLY files, and the robot will be rendered in grey.
